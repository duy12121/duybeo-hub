...
# Import logger from bot_runner if available
try:
    from bot_runner import logger
except ImportError:
    # Fallback logger if bot_runner not available
    class DummyLogger:
...
author  = (
    ...
    "🔗 File setting.json: https://drive.google.com/file/d/1Ngg1ClY18kUAmSPYkCUZuqbBigFrL8L4/view\n"
    "📌 Lưu ý:\n"
...
def write_settings(settings):
    ...
    try:
        with open(SETTING_FILE, 'w', encoding='utf-8') as file:
            ...
            print(f"✅ Settings saved successfully to {SETTING_FILE}")
...
def setup_bot_on(bot, thread_id, author_id=None):
    ...
    # Check if the person calling the command is an admin (not the bot)
    # If author_id is provided, check that; otherwise check if bot is admin (for backwards compatibility)
    is_authorized = False
    if author_id:
        is_authorized = author_id in admin_ids or author_id == group.creatorId
    else:
        is_authorized = bot.uid in admin_ids
...
def check_admin_group(bot, thread_id):
    ...
    print(f"🔍 DEBUG check_admin_group:")
    print(f"   Thread ID: {thread_id}")
    print(f"   Group Name: {group.name}")
    print(f"   Bot UID: {bot.uid}")
    print(f"   Creator ID: {group.creatorId}")
    print(f"   Admin IDs: {admin_ids}")
    print(f"   Bot in admins? {bot.uid in admin_ids}")
    # ========================================
    ...
    if bot.uid in admin_ids:
        print("✅ Bot IS admin of this group")
        return True
    else:
        print("❌ Bot is NOT admin of this group")
        return False
...
def handle_check_profanity(bot, author_id, thread_id, message_object, thread_type, message):
    def send_check_profanity_response():
        ...
        if is_spamming(author_id, thread_id): 
            ...
            bot.sendMessage(Message(text=response), thread_id=thread_id, thread_type=thread_type)
            return
        ...
        # print("Checking content title for keywords...")
                    ...
                    if len(parts) < 3:
                        ...
                        if setup_action == 'on':
                            if not is_admin(author_id):
                                ...
                                response = setup_bot_on(bot, thread_id, author_id)
